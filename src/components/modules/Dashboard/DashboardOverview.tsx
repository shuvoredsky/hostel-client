...
export default function DashboardOverview({ role }: DashboardOverviewProps) {
  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      try {
        setIsLoading(true);
        const response = await browserClient.get(`/dashboard/${role}?range=all`);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-slate-800 rounded-2xl p-5 border border-slate-200 dark:border-slate-700">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-slate-900 dark:text-white">Active Bookings</h3>
            <Link
              href={`${routeBase}/my-bookings`}
          ...
          <div className="space-y-3 mb-4">
            <div className="flex items-center justify-between p-3 rounded-xl bg-emerald-50 dark:bg-emerald-900/20">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-slate-700 dark:text-slate-300">Total Paid</span>
              </div>
            ...
            <div className="flex items-center justify-between p-3 rounded-xl bg-amber-50 dark:bg-amber-900/20">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-slate-700 dark:text-slate-300">Total Due</span>
              </div>
              ...
              {payments.unpaidDetails.slice(0, 2).map((payment: any) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-slate-400">{payment.daysWaiting} days waiting</p>
                  </div>
                  <Link
                    href={`${routeBase}/my-payments`}
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl p-5 border border-slate-200 dark:border-slate-700">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold text-slate-900 dark:text-white">Recent Booking History</h3>
          <Link
            href={`${routeBase}/my-bookings`}
            className="text-xs text-emerald-600 hover:text-emerald-700"
          >
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-200 dark:border-slate-700">
                  <th className="text-left py-2 text-xs font-medium text-slate-500 uppercase">Listing</th>
                  <th className="text-left py-2 text-xs font-medium text-slate-500 uppercase">Area</th>
                  <th className="text-left py-2 text-xs font-medium text-slate-500 uppercase">Status</th>
                  <th className="text-left py-2 text-xs font-medium text-slate-500 uppercase">Payment</th>
                  <th className="text-left py-2 text-xs font-medium text-slate-500 uppercase">Date</th>
                </tr>
              </thead>
              <tbody>
                {bookingHistory.slice(0, 5).map((booking: any) => (
                  ...
                  >
                    ...
                    <td className="py-3 text-slate-500 dark:text-slate-400">{booking.area}</td>
                    <td className="py-3">
                      <span
